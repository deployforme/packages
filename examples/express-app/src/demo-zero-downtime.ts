...
async function main(): Promise<void> {
  console.log('\n' + '='.repeat(80));
  console.log('  Hivelet Runtime Module Management - Zero Downtime Demo');
  console.log('='.repeat(80) + '\n');

  log.info('Initializing Hivelet kernel...');
  ...
  await kernel.start();
  log.success('Kernel initialized with monitoring dashboard');
  ...
  try {
    ...
    log.error(`Module loading failed: ${message}`);
    throw error;
  }
  ...
  app.post('/admin/reload/:module', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      const duration = Date.now() - startTime;
      log.error(`Module '${moduleName}' reload failed after ${duration}ms: ${message}`);
      res.status(500).json({
        success: false,
        error: message,
        timestamp: new Date().toISOString()
  ...
  app.listen(3001, () => {
    ...
    console.log('  Status:   GET  http://localhost:3001/admin/status');
...
main().catch(error => {
  const message = error instanceof Error ? error.message : String(error);
  log.error(`Server startup failed: ${message}`);
  process.exit(1);
});
